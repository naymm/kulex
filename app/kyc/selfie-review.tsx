...
export default function KycSelfieReview() {
  const insets = useSafeAreaInsets();
  const { selfieUri } = useLocalSearchParams<{ selfieUri?: string }>();
  const uri = useMemo(() => (typeof selfieUri === 'string' ? selfieUri : ''), [selfieUri]);
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom, 18) }]}>
        <Pressable
          ...
          onPress={() => router.replace('/kyc/video-selfie-loading')}
        >
        ...
        <Pressable
          ...
          accessibilityRole="button"
          onPress={() => router.replace('/kyc/selfie-capture')}
        >
          <Text style={styles.outlineText}>Refazer a foto</Text>
        </Pressable>
      </View>
    </View>
  );
