...
export default function ReferencePaymentConfirmScreen() {
  const insets = useSafeAreaInsets();
  const { entity, reference, amount } = useLocalSearchParams<{
    entity?: string;
    reference?: string;
    amount?: string;
  }>();

  const paymentAmount = useMemo(() => parsePaymentAmountFromFields({ amount }), [amount]);
  const {
    fundingSource,
    setFundingSource,
    accountId,
    ...
    <View style={styles.container}>
      ...
      <ScrollView
        ...
        <View style={styles.card}>
          <SummaryRow label="Tipo" value="Pagamento Por Referência" />
          <SummaryRow label="Entidade" value={entity ?? '—'} />
          <SummaryRow label="Referência" value={reference ?? '—'} />
          <SummaryRow label="Montante" value={amount ? `AOA ${amount}` : '—'} />
      ...
      <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom, 16) }]}>
        <Pressable
          ...
          onPress={() =>
            router.push({
              pathname: '/payments/referencia/pin',
              params: {
                entity: entity ?? '',
                reference: reference ?? '',
                amount: amount ?? '',
