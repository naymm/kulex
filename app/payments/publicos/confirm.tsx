...
export default function PublicServiceConfirmScreen() {
  const insets = useSafeAreaInsets();
  const { provider, providerLabel, product, value, customerNumber } = useLocalSearchParams<{
    provider?: string;
    ...
    <View style={styles.container}>
      ...
      <ScrollView
        ...
        <View style={styles.card}>
          <SummaryRow label="Serviço" value={providerLabel ?? '—'} />
          <SummaryRow label="Produto" value={product ?? '—'} />
          <SummaryRow label="Valor" value={value ?? '—'} last />
        </View>

        <View style={styles.card}>
          <SummaryRow label="Número de cliente ou contrato" value={customerNumber ?? '—'} />
      ...
      <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom, 16) }]}>
        <Pressable
          ...
          onPress={() =>
            router.push({
              pathname: '/payments/publicos/pin',
              params: {
