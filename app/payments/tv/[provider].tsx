...
import {
  getDefaultTvForm,
  getTvProvider,
  isValidTvCustomerNumber,
  TV_PRODUCTS,
  type TvProviderId,
} from '@/constants/tv-streaming';
...
export default function TvProviderScreen() {
  ...
  const resolvedProviderId = (provider?.id ?? 'zap') as TvProviderId;
  const products = TV_PRODUCTS[resolvedProviderId];
  const defaults = useMemo(() => getDefaultTvForm(resolvedProviderId), [resolvedProviderId]);
  ...
  const canContinue = isValidTvCustomerNumber(customerNumber);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView
        ...
        />
        <PaymentTextField
          label="Número de cliente ou cartão"
          value={customerNumber}
          placeholder="Número cliente ou cartão"
