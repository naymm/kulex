...
export default function EstadoPaymentConfirmScreen() {
  const insets = useSafeAreaInsets();
  const { reference, amount, amountDigits } = useLocalSearchParams<{
    reference?: string;
    amount?: string;
    amountDigits?: string;
  }>();
  ...
  const paymentAmount = useMemo(
    () => parsePaymentAmountFromFields({ amount, amountDigits }),
    [amount, amountDigits],
  );
  const {
    fundingSource,
    ...
    <View style={styles.container}>
      ...
      <ScrollView
        ...
        <View style={styles.card}>
          <SummaryRow label="Tipo" value="Pagamento ao Estado" />
          <SummaryRow label="Referência RUPE" value={reference ?? '—'} />
          <SummaryRow label="Valor" value={amount ? `AOA ${amount}` : '—'} />
