...
import { kwikParamsToRoute, parseKwikParams } from '@/lib/kwik';
import { withOriginParams } from '@/lib/navigation';

export default function KwikTransferPinScreen() {
  ...
  const addDigit = (digit: string) => {
    ...
    if (next.length === 4) {
      setTimeout(() => {
        router.push({
          pathname: '/send-money/kwik/success',
  ...
  return (
    <AddMoneyShell title="Transferência KWiK">
      <View style={styles.content}>
        <Text style={styles.title}>Digite o seu PIN</Text>
        <Text style={styles.subtitle}>Para validação da transferência KWiK</Text>
